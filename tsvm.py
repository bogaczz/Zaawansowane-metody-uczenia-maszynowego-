...
X, Y = make_classification(
    n_samples=nb_samples, n_features=2, n_redundant=0, random_state=1000
)
Y[Y == 0] = -1
...
def svm_target(theta, Xd, Yd):
    ...
    return (
        (C_labeled * s_eta_labeled)
        + (C_unlabeled * s_eta_unlabeled)
        + (0.5 * np.dot(wt.T, wt))
    )
...
def labeled_constraint(theta, Xd, Yd, idx):
    ...
    c = (
        Yd[idx] * (np.dot(Xd[idx], wt) + theta[-1])
        + theta[2 : 2 + nb_samples - nb_unlabeled][idx]
        - 1.0
    )
...
def unlabeled_constraint(theta, Xd, idx):
    ...
    c = (
        theta[2 + nb_samples : 2 + nb_samples + nb_unlabeled][
            idx - nb_samples + nb_unlabeled
        ]
        * (np.dot(Xd[idx], wt) + theta[-1])
        + theta[2 + nb_samples - nb_unlabeled : 2 + nb_samples][
            idx - nb_samples + nb_unlabeled
        ]
        - 1.0
    )
...
if __name__ == "__main__":
    ...
    for i in range(nb_samples - nb_unlabeled):
        svm_constraints.append(
            {"type": "ineq", "fun": labeled_constraint, "args": (X, Y, i)}
        )
        svm_constraints.append(
            {"type": "ineq", "fun": eta_labeled_constraint, "args": (i,)}
        )

    for i in range(nb_samples - nb_unlabeled, nb_samples):
        svm_constraints.append(
            {"type": "ineq", "fun": unlabeled_constraint, "args": (X, i)}
        )
        svm_constraints.append(
            {"type": "ineq", "fun": eta_unlabeled_constraint, "args": (i,)}
        )
    ...
    ax[1].scatter(
        X[Y == -1, 0], X[Y == -1, 1], c="r", marker="o", s=100, label="Labeled class 0"
    )
    ax[1].scatter(
        X[Y == 1, 0], X[Y == 1, 1], c="b", marker="^", s=100, label="Labeled class 1"
    )
