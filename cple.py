...
if __name__ == "__main__":
    ...
    unlabeled_idx = np.random.choice(
        np.arange(0, nb_samples, 1), replace=False, size=nb_unlabeled
    )
    ...
    total_cv_scores = cross_val_score(
        LogisticRegression(
            solver="lbfgs", max_iter=10000, multi_class="auto", random_state=1000
        ),
        X,
        Y.squeeze(),
        cv=10,
    ...
    # Train CPLE
    print("Training CPLE...")
    q_end = fmin_bfgs(log_likelihood, x0=q0, maxiter=1000)
